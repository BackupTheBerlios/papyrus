 ...
 *
 */
public class EmployeeAction implements DomainAction {
	 ...
	 */
	private static Logger logger_ = Logger.getInstance(EmployeeAction.class.getName());
	...
	/**
	 * List all employees according to the form filled by the user
	 * @param psession 
	...
	public String listAction(HttpSession psession, HashMap pparametersMap) throws ServletException, IOException, PapyrusException {
		...
		ItemListBean employeeListBean = null;
				
		/* reset the associated form bean and setup it with the current form */
		FormMappingObject formBean = (FormMappingObject) psession.getAttribute("employeeListForm");
		if (null == formBean)
			formBean = FormMappingFactory.getInstance().getFormMappingObject("Employee", "LIST");
		else
			formBean.reset();
		...
		if (true == formBean.isDataOk() && "init".equals(subAction)) {
			...
			employeeListBean.search(formBean, employeeDBMappingObject.getQuery("SEARCH"));
				
			/* go to the first results page */
			employeeListBean.loadResultsFromPage(1);
		} else 
		...
		if ("goto".equals(subAction)) {
			...
			employeeListBean = (ItemListBean) psession.getAttribute("employeeListBean");
				
			if (null != employeeListBean)
				employeeListBean.loadResultsFromPage(pageIndex);
		}
			
		/* store the form in order to redisplay it */
		psession.setAttribute("employeeListForm", formBean);
			
		/* store in the request scope the list of employees */
		psession.setAttribute("employeeListBean", employeeListBean);
		
		logger_.debug("listAction : end");
		
		return ("/administration/employeeList.jsp");
	}
